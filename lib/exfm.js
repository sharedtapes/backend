...
module.exports.injectKeys = function(keys){
	exfmAPIKey = keys.exfmAPIKey;
	soundcloudAPIKey = keys.soundcloudAPIKey;
};
...
module.exports.search = function(query, start, numberOfResults){
	var d = when.defer(),
		results = [];
	if (start === undefined){
		start = 0;
	...
	request
		...
		.end(function(res){
			...
			when.all(res.body.songs.map(function(song){
				...
				// trim the length of the song result
				if (song.hasOwnProperty('title') && song.title.length > 40){
					song.title = song.title.substring(0, 40);
				}
				if (song.hasOwnProperty('artist') && song.artist.length > 40){
					song.artist = song.artist.substring(0, 40);
				}
