...
Db.prototype.updateSongSet = function(oldSongs, mixtape){
    ...
    sequence(this).then(function(next){
        ...
        // 1. add the new songs
        values = this.createSongValues(parseInt(mixtape.id, 10), songsToAdd);
        this.insertSongRecord(values).then(function(result){
            return next(result);
        }, function(err){
            return d.reject(err);
        ...
        // 3. delete songs
        this.deleteSongRecord(parseInt(mixtape.id, 10), songsToDelete).then(next, d.reject);
    }).then(function(next){
        // 4. update songs
        this.updateSongRecord(parseInt(mixtape.id, 10), songsToUpdate).then(next, d.reject);
    }).then(function(next){
        return d.resolve();
    });
    return d.promise;
