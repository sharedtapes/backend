...
Db.prototype.insertSongRecord = function(values){
    var d = when.defer();
    this.client.query("INSERT INTO songs (mixtape_id, exfm_song_id, title, artist, album, url, track_order) VALUES " +
...
Db.prototype.selectSongsByMixtapeId = function(id){
    var d = when.defer();
    this.client.query("SELECT song_id, title, artist, album, url, track_order " +
...
Db.prototype.selectSimilarArtistsById = function(songs){
    var d = when.defer(),
        whereClause = "";
    for (var i = 0; i < songs.length; i++){
        whereClause += "song_id = '" + songs[i].id + "'";
        if (i !== songs.length - 1){
            whereClause += " OR ";
        }
    }
