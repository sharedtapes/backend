...
Db.prototype.updateMixtape = function(id, tape){
    ...
    mixtape.id = id;
    mixtape.base10Id = parseInt(mixtape.id, 36);
    ...
    sequence(this).then(function(next){
        ...
        this.selectSongsByMixtapeId(mixtape.base10Id).then(function(result){
            return next(result);
        }, function(err){
            this.client.query("ROLLBACK", function(err){
                return d.reject(err);
...
Db.prototype.getMixtape = function(id){
    var d = when.defer(),
        ...
        songObject = {};
    this.selectMultipleJoin(parseInt(id, 36)).then(function(result){
        mixtape.title = result.rows[0].mixtape_title;
        ...
        for (var i = 0; i < result.rows.length; i++){
            ...
            if (mixtape.songs.indexOfSimilar(songObject) === -1){
                mixtape.songs.push(songObject);
            }
            if (mixtape.similarArtists.indexOf(result.rows[i].similar_artist) === -1){
                mixtape.similarArtists.push(result.rows[i].similar_artist);
            }
        }
        return d.resolve(mixtape);
    }, d.reject);
...
Db.prototype.updateMixtapeRecord = function(mixtape){
    ...
    this.client.query("UPDATE mixtapes SET title=$1, mod_date=$2, locked=$3 " +
        ...
        mixtape.locked, mixtape.base10Id], function(err){
            if (err){
                return d.reject(err);
            }
            return d.resolve();
...
Db.prototype.updateSongSet = function(oldSongs, mixtape){
    ...
    sequence(this).then(function(next){
        ...
        // 3. delete songs
        this.deleteSongRecord(mixtape.base10Id, songsToDelete).then(next, d.reject);
    }).then(function(next){
        // 4. update songs
        this.updateSongRecord(mixtape.base10Id, songsToUpdate).then(next, d.reject);
    }).then(function(next){
        return d.resolve();
    });
    return d.promise;
